...
from customer360.api.portfolio_analytics import (
    _review_kpis_for_segment,
...
def test_review_kpis_on_portfolio_payload(conn: sqlite3.Connection) -> None:
    data = fetch_portfolio_analytics(conn, segment="customers_all")
    kpis = data["kpis"]
    assert "avg_review_rating" in kpis
    assert "review_count" in kpis
    assert isinstance(kpis["review_count"], int)
    if interactions_table_exists(conn):
        review = _review_kpis_for_segment(conn, "customers_all")
        assert kpis["review_count"] == review["review_count"]
        assert kpis["avg_review_rating"] == review["avg_review_rating"]
